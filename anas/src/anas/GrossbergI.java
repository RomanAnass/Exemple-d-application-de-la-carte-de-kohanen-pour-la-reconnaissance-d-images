...
public void TrainI(int nembreneuron , int dimMap ,int iterMax , double errMax ,double tau ,String data1, String data2 ) throws FileNotFoundException, IOException{
...
do{
    ...
    out1 = new PrintWriter(new FileWriter("data1.txt"),true);
    ...
    for(int i = 0 ; i < data_rows ; i++){
       ...
       Winner = som.Train_k(input_vector,iter) ;
       ...
       for(int k1 = 0 ; k1 < nembreneuron ; k1++ ){
            ...
            //boucle des poids des neurones de grossberg :            
             for(int t = 0 ; t < nombrepoids ; t++){
                ...
                for(int w = 0 ; w < data_cols ; w++){
                    
                    k[t] += som.Map[t].Weight_of_Node[w]*input_vector[w];
                } 
                 
                 
             }
         // modification de poids de neurone de grossberg :      
         ca[k1].Update_Weight_gI( ViteesApp , k , yd , k1 , nombrepoids );
...
//iter < iterMax ||
}while( iter < iterMax && errMo_g > errMax && erreurMo_k > errMax );
  
// System.out.println("fin d'qpprentissage");

// pour enregistrer les poids finales des neurones de kohanen : 
 out2 = new PrintWriter(new FileWriter("poids_finale_kI.txt"),true);
...
out3 = new PrintWriter(new FileWriter("poids_finale_gI.txt"),true);  

out3.println("************************************** LES POIDS FINALES DES NEURONES DE GROSSBERG ********************************");         

for(int i = 0 ; i < nembreneuron ; i++ ){
    
    out2.println("neurone :"+i);
...
out4 = new PrintWriter(new FileWriter("resultatI.txt"),true);
